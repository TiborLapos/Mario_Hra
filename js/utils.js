...
function collisionTop( {objec1, objec2}){
    return (objec1.position.y + objec1.height <= objec2.position.y && objec1.position.y + objec1.height + objec1.velocity.y >= objec2.position.y && objec1.position.x + objec1.width >= objec2.position.x && objec1.position.x <= objec2.position.x + objec2.width)
}

function hitBotomOfPLatform({object, platform}){
    return (object.position.y<= platform.position.y + platform.height && object.position.y - object.velocity.y >= platform.position.y + platform.height && object.position.x + object.width >= platform.position.x && object.position.x <= platform.position.x + platform.width /*- 50*/)
}
